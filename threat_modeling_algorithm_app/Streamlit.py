...
def add_threats_to_puml(content, threats_df):
    ...
    def replace_threat(match):
        ...
        if not threat_row.empty:
            ...
            new_details = f'{details}, Threat: {threat}'
            return f'Rel({source}, {target}, "{relationship}", "{new_details}")'
        return match.group(0)  # Return the original match if no threat found
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if not df.empty:
            ...
            model, encoder = load_model('RF_algorithm_1.0.4.pkl')
